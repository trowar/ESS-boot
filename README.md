...
本项目用于制作云主机镜像。它在原主机安装一套开机控制和只读 rsync 环境；使用该镜像创建的新机器开机后，会自动识别自己不是原主机，从原主机取得最新的 `/srv/run.sh`，然后执行其中的业务命令。
...
```text
       ...
       ┌───────────────┼────────────────┐
       │               │                │
 /srv/boot.sh     /srv/run.sh      rsync只读服务
  开机控制脚本      业务脚本          TCP 873
       │                                │
       │ 随镜像复制                     │ 提供最新run.sh和文件
       ▼                                │
...
| `/srv/boot.sh` | 开机控制脚本。等待网络、判断机器身份、安全同步并执行 `run.sh`。由 `install.sh` 自动生成。 |
| `/srv/run.sh` | 用户维护的业务脚本。填写伸缩实例开机后真正需要执行的命令。 |
| `/etc/rc.local` | Ubuntu/Debian 的开机入口。仅在缺少调用时追加 `boot.sh`。 |
| `/etc/rc.d/rc.local` | CentOS/Rocky/AlmaLinux 的开机入口。 |
| `/etc/rsyncd.conf` | 原主机 rsync 服务配置。模块名固定为 `root`，只读映射根目录 `/`。 |
| `/etc/rsync.pwd` | 伸缩实例连接原主机 rsync 服务使用的令牌，权限为 `600`。 |
| `/etc/rsync.secret` | 原主机 rsync 服务端认证文件，权限为 `600`。 |
| `/var/log/scale-boot.log` | 独立开机日志，记录 `boot.sh` 和 `run.sh` 的标准输出与错误。 |
...
```bash
rsync -az --timeout=60 --contimeout=15 --password-file=/etc/rsync.pwd root@${RSYNC_IP}::root/srv/run.sh /srv/run.sh.tmp
```
...
`exec` 让 `run.sh` 接管当前进程，因此 `run.sh` 的输出和错误会继续进入 `/var/log/scale-boot.log`，不需要在 `run.sh` 内重复设置日志。

## 安全更新 run.sh

`boot.sh` 不会直接覆盖正在使用的 `/srv/run.sh`，而是执行：
...
```bash
sh /srv/boot.sh --ignore-ip
```

正常开机调用不能添加 `--ignore-ip`，否则原主机也会同步并执行 `run.sh`。

测试可执行脚本同步时，可以在原主机准备 `/srv/rsync-test.sh`，并在 `run.sh` 中写入：
...
```bash
cat /var/log/scale-boot.log
```

实时查看：

```bash
tail -f /var/log/scale-boot.log
```
...
- 已配置的 rsync 模块和服务会跳过。
- 已开放的防火墙端口会跳过。
- `/srv/run.sh` 已存在时不会覆盖用户内容。
- `rc.local` 已包含相同调用时不会修改或重复追加。

## 安全说明

- rsync 使用 TCP `873`，云平台安全组需要允许伸缩实例访问原主机该端口。
- rsync 模块 `root` 以只读方式映射原主机根目录 `/`，可读取原主机全部路径。
- 必须通过安全组或防火墙限制 TCP 873 的来源，只允许可信的伸缩实例访问。
- 必须保护 `/etc/rsync.pwd` 和 `/etc/rsync.secret`，不要输出、上传或提交到代码仓库。
